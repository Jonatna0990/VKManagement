...
namespace StorageProvider.Image
{
    public sealed partial class CacheImage : UserControl
    {
            ...
            await CoreWindow.GetForCurrentThread().Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
            {
                ...
                if (control != null && imageUri != null)
                {
                    ...
                    if (imageUri.Scheme == "http" || imageUri.Scheme == "https")
                    {
                        ...
                        bool fileExists = await StorageHelper.FileExists(filename,CacheType.Image);
                        if (fileExists)
                        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    using (var response = await client.GetAsync(imageUri))
                    {
                        ...
                        // save image
                        await StorageHelper.WriteFile(GetFileNameInIsolatedStorage(imageUri), CacheType.Image, imageBuffer);
        ...
        private async void LoadFromLocalStorage(Uri imageUri, BitmapImage bitmap)
        {
            try
            {
                ...
                var buffer = await StorageHelper.ReadFile(isolatedStoragePath,CacheType.Image);
        ...
        {
            var hash = EncryptionUtils.GetSha1(uri.AbsoluteUri);
            return hash;
        }
    }
